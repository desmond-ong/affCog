...
#source_url("https://raw.github.com/desmond-ong/common/master/R/summarySE.R")
...
# # also storing the CI of participants
# model$ObservedPosterior1ci <- (summarySE(expt2_data, measurevar="outcomeEstimate1", groupvars=c("spinnerID"))$ci)/8 
# model$ObservedPosterior2ci <- (summarySE(expt2_data, measurevar="outcomeEstimate2", groupvars=c("spinnerID"))$ci)/8
# model$ObservedPosterior3ci <- (summarySE(expt2_data, measurevar="outcomeEstimate3", groupvars=c("spinnerID"))$ci)/8

# Normalizing the observed posteriors and cis
...
# model$ObservedPosterior1ci <- model$ObservedPosterior1ci/model$ObservedPosteriorSum
# model$ObservedPosterior2ci <- model$ObservedPosterior2ci/model$ObservedPosteriorSum
# model$ObservedPosterior3ci <- model$ObservedPosterior3ci/model$ObservedPosteriorSum
...
# Here, we just set the number of iterations to 1, and use the non-bootstrapped dataframe. 
...
#modelResults = matrix(rep(NA,150), nrow=150)
...
for(k in 1:numberOfIterations) { # bootstrap loop
  ...
  #modelResults1 = matrix(rep(NA,150), nrow=150)
  ...
  for(j in 1:40) { # Test variance smoothing from 1.1 to 5.0. Going down the trials
    ...
    #modelResults1 = cbind(modelResults1, EstPosterior)
  ...
  #modelResults1 = modelResults1[,-1]
  #modelResults = cbind(modelResults, modelResults1[,which.min(modelRMSE$RMSE)])
...
plotDF = data.frame(modelMean = apply(modelResults,1,mean),
                    modelSD = apply(modelResults,1,sd),
                    observerMean = ObsPosterior,
                    observerCI = c(model$ObservedPosterior1ci, model$ObservedPosterior2ci, model$ObservedPosterior3ci))

#modelResults$mean = rowMeans(modelResults)
#modelResults$SD = rowSD

ggplot(plotDF, aes(x=modelMean, y=observerMean)) +
  geom_point() +
  geom_smooth(method="lm") +
  #geom_abline(intercept = 0, slope = 1) +
  geom_errorbar(aes(ymin=observerMean-observerCI, ymax=observerMean+observerCI)) +
  geom_errorbarh(aes(xmin=modelMean-modelSD, xmax=modelMean+modelSD)) +
  xlab("Model Predictions") + ylab("Participants' posterior estimates") +
  coord_cartesian(xlim=c(0, 0.85), ylim=c(0, 0.85)) +
...
ggplot(plotDF, aes(x=modelMean, y=observerMean)) +
  ...
  #geom_smooth(method="lm") +
  geom_abline(intercept = 0, slope = 1, size=1.0, linetype="dashed", color="red") +
  geom_errorbar(aes(ymin=observerMean-observerCI, ymax=observerMean+observerCI), color = "white") +
  geom_errorbarh(aes(xmin=modelMean-modelSD, xmax=modelMean+modelSD), color = "white") +
  xlab("Model Predictions") + ylab("Participants' posterior estimates") +
  ...
  theme(legend.position="top",
        ...
        axis.title.x = element_text(size=14, vjust=-0.2, color = "white"),
        axis.title.y = element_text(size=14, vjust=0.8, color = "white"),
...
![Figure 6. (A) Simple causal model. The solid arrow represents a causal relationship (outcomes “cause” emotion; P(e|o) from Experiment 1), while the dashed arrow represents an inference that can be made (P(o|e) from Experiment 2). (B) Comparison of participants’ estimates of the posterior probability from Experiment 2 with the predictions of the model built in Experiment 1. There is a strong correlation of 0.806 (0.792, 0.819). The dashed red line has intercept 0 and slope 1 and is added for reference. (For interpretation of the references to colour in this figure legend, the reader is referred to the web version of this article.](writeup/Fig6_exp2Results.pdf)
...
# # black:
# ggplot(expt3expected, aes(x=expectedBayes, y=expectedEmpirical)) + geom_point(size=0.5, color = "white") +
#   geom_abline(intercept = 0, slope = 1, size=1.0, linetype="dashed", color="red") + # "y=x" line
#   #geom_smooth(method="lm") +
#   scale_y_continuous(limits=c(1, 9)) + scale_x_continuous(limits=c(1, 9)) + 
#   xlab("Bayesian Model Predictions") + ylab("Participants' emotion attribution") +
#   theme(legend.position="top",
#         strip.background = element_rect(fill="#FFFFFF"),
#         strip.text = element_text(size=12),
#         axis.line = element_blank(),
...
# ggplot(expt4expected, aes(x=expectedBayes, y=expectedEmpirical)) + geom_point(size=0.5, color = "white") +
#   geom_abline(intercept = 0, slope = 1, size=1.0, linetype="dashed", color="red") + # "y=x" line
#   #geom_smooth(method="lm") +
#   scale_y_continuous(limits=c(1, 9)) + scale_x_continuous(limits=c(1, 9)) +
#   xlab("Bayesian Model Predictions") + ylab("Participants' emotion attribution") +
